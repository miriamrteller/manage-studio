-- =============================================================================
-- Waiver Evidence (V1 self-hosted)
-- DEPENDENCIES: 001 (user_profiles, helper fns), 002 (people, account_members), 006 (audit_log), 008 (consent_templates)
-- Immutable evidence rows + append-only event log.
-- All writes go through the sign_waiver() SECURITY DEFINER RPC (service_role only).
-- =============================================================================
...
CREATE TABLE waiver_evidence (
  ...
  record_hmac           VARCHAR(64) NOT NULL,  -- hmac-sha256-hex over canonical 14-field JSON (see SPEC §4.2.9)
  ...
  -- Effective supersession is determined by checking consent_version against the current active template.
  -- 'superseded'/'revoked' reserved for future admin actions via a separate mechanism.
...
CREATE TABLE waiver_events (
  id                  UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id           UUID        NOT NULL REFERENCES tenants(id),
  -- NULL for 'viewed' events that precede evidence creation; NOT NULL for accepted/superseded/revoked
...
-- waiver_evidence SELECT: own tenant + own person or family members, or admin
...
-- Concurrency: uses INSERT ... ON CONFLICT DO NOTHING to avoid SELECT-then-INSERT race.
-- =============================================================================
...
BEGIN
  ...
  -- If the row already existed (idempotent replay), fetch its id
  ...
  -- audit_log column is actor_id (not actor_user_id — verified in migration 006)
...
-- =============================================================================
-- Storage: waiver-pdfs bucket (private — only service_role writes, admins read)
-- Object path pattern: {tenant_id}/{person_id}/{evidence_id}.pdf
-- =============================================================================
...
DROP POLICY IF EXISTS "Admins delete own tenant waiver PDFs" ON storage.objects;

-- Edge Function (service_role) can INSERT, UPDATE, DELETE
...
-- Admins can read (SELECT) PDFs belonging to their tenant
-- Path: {tenant_id}/{person_id}/{evidence_id}.pdf  →  folder[1] = tenant_id
...
-- Parents / students can read their own waiver PDFs
-- Path folder[2] = person_id
CREATE POLICY "Users read own waiver PDFs"
  ON storage.objects FOR SELECT
  USING (
    bucket_id = 'waiver-pdfs'
    AND (
      -- person is the authenticated user's linked person
      (storage.foldername(name))[2] = (
        SELECT person_id::text FROM public.user_profiles WHERE id = auth.uid()
      )
      -- or person belongs to the authenticated user's family
