-- =============================================================================
-- 003000: Feature flag system
-- Adds: tenant_plan ENUM, verticals lookup table, tenants.plan + tenants.skin columns,
--       feature_definitions, tenant_feature_overrides, get_tenant_features(),
--       provision_tenant() (replaces 002400 version with new plan/vertical signature),
--       get_tenant_config_by_subdomain() (replaces 001800 version, adds plan/skin/features).
-- DEPENDENCIES: 000200, 001600, 002400
-- =============================================================================

-- =============================================================================
-- STEP 1 — tenant_plan ENUM
-- Binary. Essential = appointment-based, single or deposit+payment across clients.
-- Professional = class enrollment, calendar-accurate terms, multi-child billing families.
-- =============================================================================
CREATE TYPE tenant_plan AS ENUM ('essential', 'professional');

-- =============================================================================
-- STEP 2 — verticals lookup table
-- New verticals are a single INSERT — never a migration.
-- plan_restriction: if set, vertical is only available on that plan. NULL = any plan.
-- =============================================================================
CREATE TABLE verticals (
  id             TEXT        PRIMARY KEY,
  display_name   TEXT        NOT NULL,
  plan_restriction tenant_plan,          -- NULL = available on all plans
...
INSERT INTO verticals (id, display_name, plan_restriction) VALUES
  ...
  ('generic',       'General',            NULL);         -- DB fallback; not shown in picker

-- =============================================================================
-- STEP 3 — extend tenants table
-- skin  = vertical id (skin IS the vertical — one FK, not two columns).
-- plan  = access tier.
-- =============================================================================
ALTER TABLE tenants
  ADD COLUMN IF NOT EXISTS plan             tenant_plan   NOT NULL DEFAULT 'essential',
  ADD COLUMN IF NOT EXISTS skin             TEXT          NOT NULL DEFAULT 'generic'
