 ...
 */
export const SHADE_FLAGS = flags.CHAG |
    flags.MINOR_HOLIDAY |
 ...
 */
export const SKIP_FLAGS = flags.CHAG |
    flags.MAJOR_FAST |
    flags.MINOR_FAST |
    flags.CHOL_HAMOED |
    flags.CHANUKAH_CANDLES;
...
function skipsSchedule(ev) {
    const eventFlags = ev.getFlags();
    // Studio override: always cancel regardless of SKIP_FLAGS mask
    if ((eventFlags & SHADE_FLAGS) !== 0 && ALWAYS_SKIPPED_DESCS.has(ev.getDesc()))
        return true;
    if ((eventFlags & SKIP_FLAGS) === 0)
        return false;
    if ((eventFlags & flags.YOM_KIPPUR_KATAN) !== 0)
        return false;
...
export function getHolidayFlags(date) {
    return eventsOn(date).reduce((mask, ev) => ((ev.getFlags() & SHADE_FLAGS) !== 0 ? mask | ev.getFlags() : mask), 0);
}
...
export function isHolidayOrErev(date) {
 ...
 */
export function isHolidayInIsrael(instant) {
...
export function getHolidayDates(start, end) {
    const out = [];
...
export function getSkippedHolidayDates(start, end) {
    const out = [];
    const last = civilDay(end);
    for (const current = civilDay(start); current <= last; current.setDate(current.getDate() + 1)) {
        if (isSkippedHoliday(current))
            out.push(new Date(current));
    }
    return out;
}
...
export function categoryFromFlags(eventFlags) {
    if (eventFlags & flags.CHAG)
        return 'major';
    if (eventFlags & flags.MAJOR_FAST)
        return 'major_fast';
    if (eventFlags & flags.MINOR_FAST)
        return 'minor_fast';
    if (eventFlags & flags.CHOL_HAMOED)
        return 'chol_hamoed';
    if (eventFlags & flags.CHANUKAH_CANDLES)
        return 'chanukah';
