...
if (!response.ok) {
  ...
  if (
    ...
    console.log(`Auth user already exists: ${PARENT_EMAIL} (${PARENT_AUTH_USER_ID})`);
    process.exit(0);
  }
  console.error('Failed to create auth user:', message);
  process.exit(1);
}

console.log(`Created auth user: ${body.email ?? PARENT_EMAIL} (${body.id ?? PARENT_AUTH_USER_ID})`);
console.log('Re-run supabase/seed.sql to sync user_profiles and family links if needed.');
