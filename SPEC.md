...
| -------------- | -------------- | --------------------------------------------------- |
| Cloudflare Workers (static assets) | Platform owner | Central SPA host; serves all tenants via wildcard subdomain (CDN/DNS/WAF on Cloudflare). **Not Pages** — Pages does not support wildcard custom domains. Wildcard requires a Workers *route* (`*.opalswift.com/*`) plus a proxied wildcard DNS record; Custom Domains do not match wildcards. Universal SSL covers the apex and all first-level subdomains at no cost. |
...
| Docker                     | Not needed; Cloudflare Workers + Supabase handle deployment                            |
...
LEGAL
[ ] Privacy Policy + Terms linked (footer: set VITE_PRIVACY_POLICY_URL + VITE_TERMS_URL on the Workers build / .env.local)
...
| Layer          | Method                                                                                          |
| -------------- | ----------------------------------------------------------------------------------------------- |
| Frontend       | Cloudflare Workers → Deployments → rollback to previous version (`wrangler rollback`)            |
